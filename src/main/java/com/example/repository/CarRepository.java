...
import com.example.domain.PriceStatistics;
...
public interface CarRepository extends JpaRepository<Car, Long> {
    ...
    @Query("SELECT new com.example.domain.PriceStatistics(car.brand, AVG(car.price), MIN(car.price), MAX(car.price)) " +
            "FROM Car car " +
            "GROUP BY car.brand")
    List<PriceStatistics> AvgAndMaxAndMinPricesPerBrand();
    ...
    List<PriceStatistics> AvgAndMaxAndMinPricesPerBrandOrderedByAVGPrice();


    @Query("SELECT new com.example.domain.QuantityByYear(car.yearOfFab, COUNT(car)) " +
            "FROM Car car " +
            "GROUP BY car.yearOfFab")
    List<QuantityByYear> CarsMadeByYear();
